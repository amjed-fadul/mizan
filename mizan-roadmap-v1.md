...
- **Script became a third mode dimension.** Every Arabic typography token has a Latin counterpart and none of them is a concept only Arabic has, so 007's own test makes the Arabic scale a mode rather than a parallel scale. The cost is the matrix: four combinations become eight. *This bullet originally ended "against Figma Professional's ten … any fourth dimension now has to displace something rather than join", and Stage 3 retracted it: the ten-mode limit is per **collection**, the plugin maps one collection per dimension, and the combinations are never materialised as modes. See the Stage 3 status below and the corrections in [013](decisions/013-script-is-a-mode-not-a-parallel-scale.md) and [014](decisions/014-direction-is-not-a-mode-dimension.md).*
- **Seven Decision Log entries (007–013), and the refusals are the useful ones.** 33 colours were consolidated by merging on role and verifying by distance — never the reverse. *(This read "33 colours became 14" until the shipped tokens were counted: 23 colour primitives carrying 15 colour semantics. The 14 was written before any primitive existed and never revisited — see the correction in [008](decisions/008-the-colour-consolidation.md).)* The measurement had one chance to be decisive and pointed the wrong way: the two brand greens sit 2.90 ΔE00 apart, *above* the 2.3 just-noticeable difference, so a perceptual rule leaves the most consequential merge in the system undone. Loosen it far enough to catch them and it is 1.31 away from swallowing `commerce.discount` and `mobility.safety` — *this costs less* and *this cancels your trip*. *Value coincidence is not semantic identity* stopped being a slogan the moment it cost something.
...
- **4-mode matrix:** Light/Dark × Market/Move (Professional allows 10 modes; you need 4). *Corrected in Stage 2: this line originally read Light/Dark × LTR/RTL, which was wrong. **Direction is not a mode dimension** — no token in the system takes a different value in RTL. Logical properties flip the layout and the values stay put, which is the whole reason the RTL rules mandate them. Script (Latin/Arabic) is a real third dimension, because Arabic genuinely needs different leading and a different face — but that is typography, not direction, and it is [decision 013](decisions/013-script-is-a-mode-not-a-parallel-scale.md).*
...
**Status — what the stage actually produced.** *The tooling is built and the first sync has run; the library itself is not.*

- **Done.** The Sync plugin, one-way and outward, proven idempotent against the live API — 168 changes written, re-plan clean. The drift detector with eight drift classes, each with a fixture and an assertion proven attributable. The health dashboard. The file exists with 74 variables in three collections: `primitive` (59, one mode), `theme` (12, light/dark), `product` (3, market/move) — **and the variable library is now published**, so the variables are consumable from other files rather than trapped in this one.
- **The proof sheet, which the plan did not ask for.** A generated page that draws every projected variable onto a real node and *binds* it — 74 variables across 4 mode combinations, roughly 1,072 nodes — so the claim "this token reached Figma" is a property of the document rather than a line in a report. It is the answer to the question the sync could not otherwise answer: a plan that reports zero changes is equally consistent with everything being correct and with nothing having been written. The dry run now carries **178 assertions** against it, and the sheet is a third consumer of the projection rules alongside the plugin and the detector, which is a real maintenance cost and stated as one in [015](decisions/015-rung-2-has-a-plan-floor.md).
- **The read bridge, which the plan did not know it needed.** A localhost WebSocket in the plugin that answers exactly one question — the current variable snapshot — with no create, update, rename or delete in its vocabulary. It exists because the REST read is Enterprise-gated, and it was **built rather than adopted**: [016](decisions/016-build-the-read-bridge-rather-than-adopt-figma-console-mcp.md) evaluates an existing MIT tool that does the same job with 113 more tools attached, and refuses it because a second write path to the variables would destroy the *attribution* of every finding the detector produces. Two amendments there are the honest part — the permission that shipped is narrower than the one accepted, and an adversarial review found the socket unauthenticated against any page in a browser tab, which is fixed and which nobody had thought to ask about. **Refusing a dependency does not inherit its threat model; it replaces it with your own, which nobody has reviewed.**
- Between them those two are roughly 2,200 lines of new modules before the harness that proves them, and they are the two most interesting things the stage produced. Neither was on the plan.
- **Not done.** The component library — components, published styles, in-Figma do/don'ts, a11y annotations, Community publishing. Variables are published; *components do not exist to publish*. The GIF has nothing to flip yet.
- **The finding the plan did not anticipate.** Figma gates the variables REST API in **both** directions, not just writing. So rung 2 has a plan floor: below Enterprise there is no unattended drift detection, only verification at the moment of sync. [Decision 015](decisions/015-rung-2-has-a-plan-floor.md) records the floor and what degrades below it; [016](decisions/016-build-the-read-bridge-rather-than-adopt-figma-console-mcp.md) records why we build the read path rather than adopt one.
- **Also corrected here.** The 4-mode line above originally read Light/Dark × LTR/RTL — direction is not a mode dimension ([014](decisions/014-direction-is-not-a-mode-dimension.md)). And the **ten-mode ceiling** that Stage 2 treated as a live budget is not one: the limit is per variable *collection*, the plugin maps one collection per dimension, and each collection holds two modes no matter how many dimensions exist. Both 013 and 014 carry the retraction. What that cost was three entries of arithmetic built on a constraint nobody had checked the unit of — **a limit cited without its unit is not a constraint, it is a mood.**
...
**Status — what the stage actually produced.** *Seven components, [the Storybook is live](https://amjed-fadul.github.io/mizan/), and the plan's shape held. What it did not predict is that most of the stage's difficulty was in the token layer, not the components.*

- **Seven components, and the token layer grew three times to serve them.** Button, Input, Dialog, List, Navigation, ProductCard, RideCard. Every one of them hit a value the system could not express, and each time the answer was a decision rather than a literal: control geometry that resolves by product ([022](decisions/022-control-geometry-resolves-by-product.md)), an error semantic that is **two tokens because it is gated at two thresholds** ([023](decisions/023-the-error-semantic-is-two-tokens.md)), a scrim with no contrast pairing at all ([025](decisions/025-the-scrim-is-one-value-and-carries-no-pairing.md)), and a stroke scale ([026](decisions/026-the-stroke-scale.md)). *The components were the easy half.*
- **The stroke scale is the clearest example of the stage's own failure mode.** Seven components each wrote a literal `1px`, each with a comment explaining why that was the honest answer, and each comment was right *for that component*. 023 called two occurrences "a pattern rather than an incident"; Dialog's README called five "the strongest evidence one is owed". It reached seven. **A judgment that is correct at every single step can still be wrong as a sequence**, and nothing in the process noticed, because nothing was counting.
- **The refusals.** ProductCard and RideCard stay separate ([024](decisions/024-productcard-and-ridecard-stay-separate.md)) — the flagship entry the plan predicted, and the argument came out sharper than expected: the cut is *behaviour*, not appearance, because a radio may not contain a button. And no destructive Button variant ([020](decisions/020-the-button-consolidation.md)), because `mobility.safety` is namespaced and inventing `action.danger` for one v0 button is designing a token backwards.
- **Contracts are generated by a script, not a skill.** The plan said "generate contracts with a skill you write once". `machinery/scripts/gen-contract.mjs` satisfies the intent — written once, run per component, never hand-authored — and not the letter. The authored/derived split it enforces turned out to matter more than the packaging: the generated half is build output, and the authored half is the judgment, and a review found the judgment half still telling agents a scale did not exist months after it did.
- **The Arabic defect, and the gate that had been reporting it.** Six of the seven components rendered Arabic in the Latin font stack — every component wrote `font-family.sans` and only Button overrode it. Meanwhile four `unused-primitive` warnings had been naming the unused Arabic tokens since the day they landed, inside a list of forty that nobody read. [027](decisions/027-script-is-an-overlay-not-a-dimension.md) fixed it by implementing 013 at the token layer and **corrected 013's own arithmetic on the way**: script is an overlay, not a third dimension, because no Arabic typography value depends on theme or product. *A gate that reports a real defect inside a list of noise has not reported it.*
- **What the plan did not ask for, and is the most reusable thing here: the `overlay` concept.** A mode that is deliberately not a dimension — composed on top of whichever combination is active, emitted under a selector content states, and refused if it touches anything a dimension owns. It exists because the cartesian product is the wrong shape for a mode whose values are identical across the whole matrix.
- **~~Not done: the a11y addon and the play functions never run in CI.~~ Closed.** `gates.yml` gate 7 runs every story as a test through the Vitest addon — it renders, its play function passes, and axe runs because `preview.tsx` sets `a11y.test: 'error'` globally. 58 tests, floored like every other count. **Its first run found a play function that could never have passed:** Dialog's Escape story asserted a user-agent default action, which no dispatched event produces, so it had been failing silently since the day it was written. *A check nobody runs is not a weaker check, it is a claim.* What remains: `mizan-lint` is Stage 6 work, and three decisions ([024](decisions/024-productcard-and-ridecard-stay-separate.md), [025](decisions/025-the-scrim-is-one-value-and-carries-no-pairing.md), [026](decisions/026-the-stroke-scale.md)) now owe a rule to that file. The optical Arabic size correction is still one `calc()` in Button, because a multiplier cannot be a mode value.
- **The review found more than the build did, and the worst finding was in the newest decision.** Four agents against the four pull requests. 027 shipped a `:lang(ar)` overlay with no restore, so a Latin run *inside* an Arabic page inherited the Arabic face — the exact case the entry cited as its reason for scoping to a subtree. The mechanism it replaced had handled that case correctly by accident. **An entry can be right about the architecture and wrong about the thing it is arguing from.**
