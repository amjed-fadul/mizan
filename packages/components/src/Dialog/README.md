...
- **`open` is controlled, and a consumer that ignores `onDismiss` gets a modal nobody can leave.** Escape closes the element, the `close` listener reports it upward, and if the consumer does not lower `open` the effect sees `open && !element.open` on the next render and calls `showModal()` again. The dialog reopens in the frame it closed. That is ordinary controlled-component behaviour — a controlled `<input>` that ignores `onChange` does not accept typing either — but the failure here is louder: Escape, the backdrop and the dismissing action all appear broken at once, and none of them logs anything. The `EscapeDismisses` story renders from state for exactly this reason, and its comment says so.
