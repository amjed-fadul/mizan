...
- **~~`content/rules/motion.md` is owed~~, with the rule text above. Landed:** the file exists, the reduced-motion rule is transcribed into it verbatim, and `CLAUDE.md` now points at both rule layers rather than only at RTL. Two things were added that this entry did not supply, and both are stated as additions rather than smuggled in as transcription: the rule's §4 now cites **WCAG 2.3.3**'s *essential* exemption and **2.2.2**'s explicit preload carve-out, which is the specification taking the same position on the spinner that this entry argued from first principles; and `prefers-reduced-transparency` and `prefers-contrast` are recorded as **open questions** rather than rules, because this entry did not decide them and the rule layer is not where undecided things become decided.
