...
**Until it exists, what the system actually guarantees is 48, and 48 ≥ 44 is arithmetic nothing verifies.** That is the honest state and it is stated in `control.md.min-block`'s own description rather than only here. It is also load-bearing in a specific way: Market's `md` natural height is 43.5px, so the floor is what carries the target at that one step in that one product, and it is the declaration to watch rather than the padding around it.

> **This landed.** `machinery/scripts/check-tap-target.mjs` now reads `control.*` exactly as described, and `content/tokens/targets.json` declares the footprints — the same split as `pairs.json` and the contrast gate, arrived at independently and matching. The two contexts are the two WCAG success criteria rather than one bar and an exemption: `pointer` is SC 2.5.5 (Enhanced, AAA) at 44 for a step that stands alone as an action, and `inline` is SC 2.5.8 (Minimum, AA) at 24 for `sm`, whose `$description` names the two 2.5.8 clauses — inline, and equivalent control elsewhere — that let a sub-44 control be legitimate. `sm` is not waived, it is held to the lower real bar, which is the honest reading of "declared exempt". The gate runs in `npm run check`, in the build's pre-gate sequence, and in CI with its footprint count floored the way every other coverage count is; `selftest` proves it catches a floor a mode drops under 44, a non-dimension token, a non-px unit, an unknown context, and a missing declaration. `48 ≥ 44` is no longer arithmetic nothing verifies.

**What the gate does NOT do, stated because an earlier draft of this paragraph claimed it did.** It reads the declared floors — `control.*.min-block` and `min-inline` — and nothing else. It does not read padding, font size or leading, and it has no notion of a control's natural height. So *"trim Market's `md` padding-block a rung and the gate catches it"* was false twice over: the gate never looks at padding, and `min-block-size: 48px` in the stylesheet holds the rendered height at 48 whatever the padding does, so trimming it cannot break the target in the first place. What the gate actually guarantees is narrower and worth stating exactly: **every declared footprint resolves to at least its context's bar in every mode combination** — so the failure it catches is somebody lowering a floor, or a mode resolving one below 44, not somebody changing the geometry around it. A gate that verified natural height would have to model line-height and the font's metrics, which is a different tool. An accessibility guarantee described as broader than it is, is the failure this repository exists to refuse, and it was caught in review rather than by any check.
...
- **~~`machinery/metadata/button.json` carries `status: "draft"` because of that gap.~~ Now `stable`.** The two things that held it at `draft` were the product-density work and this tap-target gate; both have landed, so `authored/button.json` moved to `stable` and the contract was regenerated rather than hand-edited. Left struck rather than deleted because the draft state was real and the record of why it lifted is the point.
...
- **~~The tap-target check is owed to `machinery/scripts/`.~~ Landed:** `machinery/scripts/check-tap-target.mjs`, declared by `content/tokens/targets.json`, floored in CI, and proven by `selftest`. See the amendment above.
