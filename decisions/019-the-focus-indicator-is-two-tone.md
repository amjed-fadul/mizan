...
- **The gate cannot tell the two kinds apart.** `pairs.json` has one `exceptions` array and both kinds land in it, so 010's watch signal is now measuring a mixture. The fix is a `kind` field — `"palette"` versus `"composite"` — so the count 010 cares about can be read on its own. That is a change to `content/tokens/`, which this entry does not own, and it is owed.
...
- **A `kind` field on exceptions is owed to `content/tokens/pairs.json`**, so 010's watch signal counts palette failures and not composite parts.
