...
If Arabic turns out to need a typographic concept Latin has no counterpart for, 007's test flips and that specific token gets namespaced while the rest stay a mode — the two mechanisms coexist by design.

The pressure is **not** the mode ceiling. This line originally said it was — *"if a fifth dimension becomes necessary, something has to give"* — which contradicts the correction above without noticing, and it is worth leaving the contradiction visible rather than quietly rewriting it, because it shows how a retracted claim survives: the correction was written into the consequence it falsified and nowhere else, and the same sentence a few lines down went on repeating it. Under collection-per-dimension a fifth dimension adds a fifth collection with two modes in it, and nothing has to give.

What a fifth dimension actually costs is the **slot indirection**, and that cost is superlinear rather than fixed. A token that varies by two dimensions already needs the selector-plus-slots pattern from [007](./007-modes-for-shared-namespaces-for-unique.md)'s amendment — two plumbing tokens for one piece of vocabulary, as `text.secondary` demonstrates. A token varying by three would need that pattern nested, and the plugin refuses such a token rather than flattening it. So the question to ask of a fifth dimension is not "is there a mode left?" but **"how many tokens will have to vary by it *and* by something else?"** — and if the answer is more than a handful, the architecture is the wrong shape regardless of what Figma permits.
