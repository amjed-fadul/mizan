...
> **Implementation gap, scoped and stated so it is not read as done.** The token layer does not support this yet. `content/tokens/modes/product.market.json` and `product.move.json` change **exactly three tokens between them, all colour** — `action.primary`, `action.primary-hover`, and the `text.secondary` slot selection. No spacing, sizing or control-geometry semantic resolves by product today; `content/tokens/primitive/dimension.json` holds a `space` ramp and a `radius` ramp with no semantic layer above them. So this half of the decision is **decided and owed**, on the same terms as [013](./013-script-is-a-mode-not-a-parallel-scale.md): the entry records the call and the token work is a separate task. Until it lands, the component ships one geometry per step and the two products' buttons are identical in size — which is a visible, temporary regression against v0, not a quiet one.
...
- **Both products' buttons are the same size until the density work lands.** The decision is taken, the mode files do not carry it yet, and the gap is visible in the running app rather than hidden in a file.
